  ...
  // connection and socket timeout for getStatus node alive check
    ...
    public TestSession getNewSession(Map<String, Object> requestedCapability) {
        ...
        try {
            ...
            host = this.getTestSlots().get(0).getRemoteURL().getHost();

            Map<String, String> param = new HashMap<String, String>();
            param.put("force", "true");

            Future steriliser = CommonThreadPool.startCallable(
            ...
            if (!steriliser.isDone()) {
                steriliser.cancel(true);
                return null;
            }

        } catch (Exception e) {
            log.warning(String.format("Error sterilising node " + host, e));
            return null;
        }
    ...
    /**
     * @throws GridException If the node if down or doesn't recognize the
     * /wd/hub/status request.
     */
