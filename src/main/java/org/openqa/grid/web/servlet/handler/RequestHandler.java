 ...
 *
 * Threading notes; RequestHandlers are instantiated per-request, run on the
 * servlet container thread. The instance is also accessed by the matcher
 * thread.
 */
...
public class RequestHandler implements Comparable<RequestHandler> {
    ...
    private long newSessionTimeout = 0;
     ...
     *
     * @throws NewSessionException in case anything wrong happens during the new
     * session process.
     */
        ...
        switch (request.getRequestType()) {
            case START_SESSION:
                ...
                try {
                    ...
                    cleanup();
                    //Q4OKR - Capture forwarding failures
                    throw new GridException("Error forwarding the new session " + e.getMessage(), e);
                }
                endTime = System.currentTimeMillis();

                CustomEnvironmentLogger environmentsLog = new CustomEnvironmentLogger();
                //Q4OKR - Capture timing trends here
                long duration = endTime - startTime;
                queueLog.recordNew("" + duration);
                if (getSession().getRequestedCapabilities().containsKey("environment")) {
                    environmentsLog.recordNew(getSession().getRequestedCapabilities().get("environment").toString());
                } else {
                    environmentsLog.recordNew("Other");
                }
            ...
            case STOP_SESSION:
                ...
                //Q4OKR - Capture stoppages here
                try {
                    forwardRequest(session, this);
                } catch (ClientGoneException e) {
                    log.warn("The client is gone for session " + session + ", terminating");
                    registry.terminate(session, SessionTerminationReason.CLIENT_GONE);
                } catch (SocketTimeoutException e) {
                    log.warn("Socket timed out for session " + session + ", " + e.getMessage());
    ...
    /**
     * calls the TestSessionListener is the proxy for that node has one
     * specified.
     *
     * @throws NewSessionException in case anything goes wrong with the
     * listener.
     */
    private void beforeSessionEvent() throws NewSessionException {
        ...
        if (p instanceof TestSessionListener) {
            try {
                ...
                log.error("Error running the beforeSessionListener : " + e.getMessage());
                e.printStackTrace();
     ...
     *
     * @throws TimeoutException if the request reaches the new session wait
     * timeout before being assigned.
     */
     ...
     * to determine where to route session-specific commands from the JSON wire
    ...
    private long getNewSessionTimeout() {
        if (newSessionTimeout != 0) {
            return newSessionTimeout;
        }
        File json = new File(RuntimeConfig.getSeleniungGridExtrasHomePath(), "hub_4444.json");

        ConfigFileReader configReader = new ConfigFileReader(json.getAbsolutePath());
        Map map = configReader.toHashMap();

        String newSessionWaitTimeout = map.get("newSessionWaitTimeout").toString();
        if (newSessionWaitTimeout.endsWith(".0")) {
            newSessionWaitTimeout = newSessionWaitTimeout.replace(".0", "");
        }
        newSessionTimeout = Long.parseLong(newSessionWaitTimeout);

        return newSessionTimeout;
    }

    public long getTimeUntilExpiration() {
        return (getNewSessionTimeout() - getWaitingTime());
    }
}
