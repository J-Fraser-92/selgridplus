...
public class ScreenshotUtility {
    ...
    public static BufferedImage getResizedScreenshot(int width, int height, boolean force) throws AWTException {
        ...
        if (width > 0 || height > 0) {
            ...
            try {
                if (force) {
                    sizedImage = Thumbnails.of(screenshot)
    ...
    public static String getResizedScreenshotAsBase64String(int width, int height)
            throws AWTException {
        BufferedImage screenshot = getResizedScreenshot(width, height, true);
        try {
    ...
    public static String getScaledScreenshotAsBase64String(int width, int height)
            throws AWTException {
        BufferedImage screenshot = getResizedScreenshot(width, height, false);
        try {
            ByteArrayOutputStream baos = writeImageToStream(screenshot);
            return encodeStreamToBase64(baos);
        } catch (IOException e) {
    ...
    public static String getFullScreenshotAsBase64String() throws AWTException {
        BufferedImage screenshot = getFullScreenshot();
        try {
            ByteArrayOutputStream baos = writeImageToStream(screenshot);
            return encodeStreamToBase64(baos);
