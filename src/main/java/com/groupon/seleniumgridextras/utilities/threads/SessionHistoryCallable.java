...
public class SessionHistoryCallable implements Callable {
    ...
    protected String notifyHubGridExtrasOfNewSession() {
        ...
        try {
            ...
            sessionLog.recordNew(getSession().getSlot().getRemoteURL().getHost(), sessionDetails);

            if(getSession().getRequestedCapabilities().containsKey("browserName")) {
                browserLog.recordNew(getSession().getRequestedCapabilities().get("browserName").toString());
            } else {
                browserLog.recordNew("Unknown");
            }
